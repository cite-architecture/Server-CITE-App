...
@JSExportTopLevel("serverciteapp.QueryObjectView")
...
def collectionListSelect = {
	...
	onchange={ event: Event => {
		...
		if (thisSelect.value == "all") {
			...
			if (ObjectModel.currentCatalog.value != None){

				val catEntry:Option[CiteCollectionDef] = ObjectModel.currentCatalog.value.get.collection(Cite2Urn(thisSelect.value))
				if (catEntry != None){
					for (p <- catEntry.get.propertyDefs) {
				...
				ObjectModel.currentCatalog.value match {
					case Some(cat) => {
						...
						cdo match {
							...
							case None => {
								QueryObjectModel.selectedPropertyType.value = None
							}
						}
					}	
					case None => {
