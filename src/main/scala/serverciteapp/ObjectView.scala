...
@JSExportTopLevel("serverciteapp.ObjectView")
object ObjectView {
...
def propertyLabelFromPropertyUrn(urn:Cite2Urn) = {
	...
	val collDef:Option[CiteCollectionDef] = ObjectModel.currentCatalog.value.get.collection(collectionUrn)
	collDef match {
		case Some(cd) => {
...
def cursorWaiting = {
	js.Dynamic.global.document.getElementById("object_Container").setAttribute("class","waiting")
}
def cursorNormal = {
	js.Dynamic.global.document.getElementById("object_Container").setAttribute("class","")
}
